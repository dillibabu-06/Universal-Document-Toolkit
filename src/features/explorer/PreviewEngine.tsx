...
import ExcelPreviewer from './previewers/ExcelPreviewer';
...
export default function PreviewEngine({ file }: PreviewEngineProps) {
  // No longer generating assetUrl here as specific previewers handle it
  ...
  const isExcel = ['xlsx', 'csv'].includes(file.extension || '');

  return (
    <div className="flex flex-col h-full bg-zinc-900/50 rounded-lg overflow-hidden border border-border-dark shadow-inner">
      <div className="px-3 py-2 bg-zinc-800 border-b border-border-dark flex items-center justify-between z-10 shrink-0">
      ...
      <div className="flex-1 relative overflow-hidden bg-black/20 flex flex-col">
        {isPdf ? (
          <PdfPreviewer filePath={file.path} />
        ) : isImage ? (
          <ImagePreviewer filePath={file.path} />
        ) : isDocx ? (
          <DocxPreviewer filePath={file.path} />
        ) : isExcel ? (
          <ExcelPreviewer filePath={file.path} />
        ) : (
          <div className="flex flex-col items-center justify-center text-zinc-500 p-6 text-center h-full">
            <FileArchive size={48} className="mb-4 opacity-30 text-amber-500/50" />
